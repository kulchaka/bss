...
OUT = ROOT / 'output/pdf/BSS-brandbook-v0.1.pdf'
F = ASSETS / 'fonts'
for name, file in [('Body','IBMPlexSans-Regular.ttf'),('Bold','IBMPlexSans-SemiBold.ttf'),('Display','IBMPlexSansCondensed-SemiBold.ttf')]:
...
c.setTitle('BSS / Soft BMW Service Lviv / Брендбук v0.1')
...
def text(s,x,y,size=14,font='Body',col=None):
    col=col or ('ink' if light else 'white')
    c.setFillColor(HexColor(C.get(col,col))); c.setFont(font,size); c.drawString(x,H-y-size*.8,s)
    notes.append(s)
    checks.append((n,s,x,y,pdfmetrics.stringWidth(s,font,size),size))
...
text('BSS',166,273,104,'Display');tag('СХЕМА ПРОПОРЦІЙ, НЕ МАЙСТЕР-ЛОГОТИП',92,421)
...
for i,(key,name,hx) in enumerate(swatches):
    x=48+i*174;rect(x,188,164,149,key,'line');text(name,x,355,20,'Display');tag(hx,x,386)
...
page('07 / typography','Типографіка з інженерним характером','IBM Plex: Sans Condensed для заголовків, Sans для текстів і компонентів.',True)
text('ТОЧНІСТЬ.',48,185,79,'Display')
text('У КОЖНІЙ ДЕТАЛІ.',48,271,56,'Display')
para('Сервіс BMW у Львові.\nЗрозуміло про стан автомобіля.\nУважно до кожного рішення.',49,372,465,21,col='ink')
for y,a,b in [(190,'H1 / 64-96 px','Condensed SemiBold / 1.0'),(259,'H2 / 36-56 px','Condensed SemiBold / 1.1'),(328,'BODY / 16-18 px','Sans Regular / 1.5'),(397,'LABEL / 12-14 px','Sans SemiBold / 1.3')]:
    tag(a,617,y);text(b,617,y+26,15,'Body','ink')
para('Українські символи: Ґґ Єє Іі Її. На мобільному H1: 40-52 px, H2: 28-36 px. Великі літери - для коротких заголовків. Суцільний текст - звичайний регістр.',48,495,850,13,col='ink')
c.linkURL('https://github.com/IBM/plex',(616,H-464,912,H-175),relative=0)
...
rect(48,218,293,113,'white');text('СЕРВІС BMW',64,239,33,'Display','ink');tag('LVIV / BSS',65,296)
...
for i,(state,title) in enumerate([('DEFAULT','Діагностика'),('HOVER / FOCUS','Діагностика'),('SELECTED','Діагностика')]):
    ...
    tag(state,x+18,308);text('01',x+18,342,40,'Display','blue' if i else 'silver');text(title,x+18,412,27,'Display')
...
for i,(a,b,d) in enumerate(stages):
    x=48+i*296;rect(x,188,272,177,'panel');tag(a,x+18,209);text(b,x+18,265,40,'Display');rect(x+18,334,236,2,'line');rect(x+18,334,70+i*83,2,'blue');para(d,x,393,260,15)
...
text('СЕРВІСНИЙ ЛИСТ',515,306,28,'Display','ink')
...
text('Ваш BMW.',48,190,55,'Display')
text('Увага до кожної деталі.',48,255,46,'Display')
...
for i,(a,b,d) in enumerate(items):
    y=183+i*49;text(a,48,y+8,16,'Bold','blue');text(b,91,y+8,20,'Display');para(d,340,y+8,548,14);line(48,y+42,912,y+42)
...
page('16 / next steps','Основа готова до обговорення.','v0.1 фіксує запропоновану систему. Остаточні рішення приймаємо після вашого перегляду.')
...
line(48,421,912,421)
para('Джерела: два логотипи та шість ігрових зображень, надані користувачем; pear.no - референс скролінгу; IBM Plex - шрифт за OFL. BSS представлено як окремий бренд; офіційна афіліація з BMW не заявляється.',48,443,850,13)
